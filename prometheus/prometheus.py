...
def to_pandas(data):
    ...
    if result_type == 'vector':
        metrics = (metric_name(r['metric']) for r in data['result'])
        values = (np.float64(r['value'][1]) for r in data['result'])
        return pd.Series(values, index=metrics)
    elif result_type == 'matrix':
        ts = [pd.Timestamp(v[0], unit='s') for v in data['result'][0]['values']]
        metrics = [metric_name(r['metric']) for r in data['result']]
        values = [[np.float64(r['values'][n][1]) for r in data['result']] for n in range(len(ts))]
        return pd.DataFrame((pd.Series(values[n], index=metrics) for n in range(len(ts))), index=ts)
